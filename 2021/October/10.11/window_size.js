// 1. 처음부터 보여도 되는 태그라면 html 안에 만들어 주는 것이 정석 : js로 추가하지 않기
...
// 7. 함수를 변수에 줄 수 있나 ? 함수 리턴값을 어떻게 작성해야 할까? : update_size
// 피드백
// innerHTML 부터 반복되니까 그 부분도 show_size 함수에 넣으면 좋겠지 ?

const window_size = document.querySelector(".window_size");
const update_size = () =>
  (window_size.innerHTML = `monitor: ${window.screen.availWidth}, ${window.screen.availHeight} <br/> 
...
window.addEventListener("resize", () => {
  window_size.innerHTML = update_size();
});
update_size();
